...
${'Recipe Likes Query'} = mysql_query('SELECT * FROM `recipes_like` WHERE `owner` = "'.$rid.'" '.${'Sort'}.' LIMIT '.${'Number of Recommendations'}) or die(mysql_error());
if(!${'Recipe Likes Query'}){
${'Recipe Likes Query'} = mysql_query('SELECT * FROM `recipes` WHERE `owner` = "'.$rid.'" ORDER BY `add_date` DESC LIMIT 3') or die(mysql_error());
while(${'Recipe Results'} = mysql_fetch_assoc(${'Recipe Likes Query'})) {
	$d = array();
	foreach(explode(',', ${'Recipe Results'}) as $s){
		$d[] = $s;
	}
	${'Recipe Recommendations'}[] = array('title' => ${'Recipe Results'}['recipe_name'], 'thumbnail' => $d[0], 'url' => 'http://50.17.184.116/recipes/dishpics/');
}
} else{
...
while(${'Recipe Results'} = mysql_fetch_assoc(${'Recipe Likes Query'})) {
	...
	while($a < ${'Number of Recommendations'}) {
				${'Recipe Also Like'} .= '<div class="divContainer">
					<a href="../nutrition/recipeview.php'.${'Recipe Recommendations'}[$a]['url'].'">
						<div class="divGridFrame">
							<div class="divGridContent">
								<img src="/recipes/dishpics/'.${'Recipe Recommendations'}[$a]['thumbnail'].'" width="180" height="130" />
...
function getRecipes($recipe_type, $fragNum, $start, $limit) {
	...
	${'Recipe Results'} .= '<div class="div4col'.$col.'">
                                    		<a href="recipeview.php?rid='.$row['id'].'">
												<div class="divGridFrame">
													<div class="divGridContent">
														<img src="/recipes/dishpics/'.$row['username'].$row['id'].'" width="180" height="130" />
